...
function initMap() {
    ...
    barNameInput.addEventListener('input', function (e) {
        ...
        if (citySelect.value === '1') {
            googleApiSearch(e.target.value, Position.paris, barList);
        }
        else if (citySelect.value === '2') {
            googleApiSearch(e.target.value, Position.lyon, barList);
        }
        if (citySelect.value === '3') {
            googleApiSearch(e.target.value, Position.strasbourg, barList);
        }
        ...
        service.textSearch(googleSearchRequest, function (results, status) {
            if (status == google.maps.places.PlacesServiceStatus.OK) {
                ...
                nameArray.forEach(function (name, i) {
                    barList.innerHTML += "<li class='new__bar-input__prop__list__item'><div class='new__bar-input__prop__list__item__name'>" + name + "</div><div class='new__bar-input__prop__list__item__address'>" + locationArray[i] + "<img src='" + linkToPosImg + "' alt=''><div></div></div></li>";
                });
